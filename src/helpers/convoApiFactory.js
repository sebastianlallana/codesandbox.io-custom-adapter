...
function buildBody(senderId, pageId, inBody, type = 'message') {
  ...
  if (inBody.error) {
    baseMessage.error = {
      code: inBody.error.code,
      name: inBody.error.name,
      message: inBody.error.message,
    };
  }
  
  let finalMessage;
  if (type === 'delivery') {
    finalMessage = _.merge(baseMessage, { mids: inBody.mids });
  } else {
    finalMessage = _.merge(baseMessage, { text: inBody.text });
  }
...
function convoApiFactory(url, pageId, authSettings, dependencies) {
  const got = dependencies.got.extend({
    ...
    hooks: {
      ...
      beforeRequest: [
        (options) => {
          if (authSettings.authTypeInbound === 'Signature' && options.body) {
  ...
  return {
    ...
    sendDelivery: (senderId, mid, error = null) => {
      const body = buildBody(senderId, pageId, {
        mids: [
